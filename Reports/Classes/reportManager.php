...
class reportManager {
    ...
    function reportStudentsAdmin($search) {
        ...
        $selStudents = "SELECT uAccounts.id, fname, mname, lname, gfname, gmname, glname, gemail, gcontact, school, section, bday, sex FROM uAccounts, uStudents WHERE (uAccounts.id = uStudents.id AND uAccounts.isActivated = 1)";
        // Insert code block here for searching function
        if (!empty($search)) {
            $selStudents .= " AND (uAccounts.id LIKE '$search%' OR fname LIKE '%$search%' OR mname LIKE '%$search%' OR lname LIKE '%$search%' OR email LIKE '$search%')";
        }
    ...
    function reportStudentsPrincipal($schoolID, $search) {
        ...
        $selStudents = "SELECT uAccounts.id, fname, mname, lname, gfname, gmname, glname, gemail, gcontact, school, section, bday, sex FROM uAccounts, uStudents WHERE (uAccounts.id = uStudents.id AND school = $schoolID  AND uAccounts.isActivated = 1)";
        // Insert code block here for searching function
        if (!empty($search)) {
            $selStudents .= " AND (uAccounts.id LIKE '$search%' OR fname LIKE '%$search%' OR mname LIKE '%$search%' OR lname LIKE '%$search%' OR email LIKE '$search%')";
        }
    ...
    function reportStudentsTeacher($sectionID, $search) {
        ...
        $selStudents = "SELECT uAccounts.id, fname, mname, lname, gfname, gmname, glname, gemail, gcontact, school, section, bday, sex FROM uAccounts, uStudents WHERE (uAccounts.id = uStudents.id AND section = $sectionID AND uAccounts.isActivated = 1)";
        // Insert code block here for searching function
        if (!empty($search)) {
            $selStudents .= " AND (uAccounts.id LIKE '$search%' OR fname LIKE '%$search%' OR mname LIKE '%$search%' OR lname LIKE '%$search%' OR email LIKE '$search%')";
        }
