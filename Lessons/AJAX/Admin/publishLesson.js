function publishLesson(lessonID) {
    // We are going to be saving the current document to the published version and update the status of the record to 'Published'

    // Vars
    lessonTitle = document.querySelector("#lessonInTitle");
    lessonDescription = document.querySelector("#lessonInDescription");

    if (lessonTitle.value == "" || lessonTitle.value == null) {generateToast("saveError", "Notification", "Save", "Please provide a title for this lesson"); return false;} // Ensure the title is not blank
    ...
    request.open("POST", "AJAX/Admin/publishPages.php");
